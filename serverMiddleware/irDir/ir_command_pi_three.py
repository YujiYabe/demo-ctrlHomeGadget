...
# cmd R2_data_num_read 0x25 bus-read(ADR,cmd,3)
    data_numHL = bus.read_i2c_block_data(
        SLAVE_ADDRESS, R2_data_num_read, 3)  # = 0x25 #bus-read(ADR,cmd,3)
    ...
    if data_num < 65535:
        ...
        block_dat = bus.read_i2c_block_data(
            SLAVE_ADDRESS, R3_data_read, 1)  # = 0x35 #bus-read(ADR,cmd,n)
        for i in range(data_num):
            block_dat = bus.read_i2c_block_data(
                SLAVE_ADDRESS, R3_data_read, 4)  # = 0x35 #bus-read(ADR,cmd,n)
...
# cmd W2_data_num_write 0x29 bus-write(ADR,cmd,3)
    ...
    bus.write_i2c_block_data(
        SLAVE_ADDRESS, W2_data_num_write,  data_numHL)  # =
...
# main
# dir_name = '/home/pi/share/ircmd/'
# print(os.environ.get('irDir'))
dir_name = os.environ.get('irDir')
os.chdir(dir_name)
...
# -------------------180413ここから
    if (argc == 1):
        print("usage:python3 ir_command.py r 0")
        break
